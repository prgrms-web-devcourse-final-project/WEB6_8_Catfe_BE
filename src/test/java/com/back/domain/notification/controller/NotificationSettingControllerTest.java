...
class NotificationSettingControllerTest {
    ...
    class UpdateSettingsTest {
        ...
        void t1() throws Exception {
            ...
            willDoNothing().given(settingService).updateSettings(eq(userId), any());
            ...
            verify(settingService).updateSettings(eq(userId), any());
        }

        @Test
        @DisplayName("알림 설정 일괄 변경 - 빈 요청")
        void t2() throws Exception {
            // given
            UpdateSettingsRequest request = new UpdateSettingsRequest(Map.of());
        ...
        @Test
        @DisplayName("알림 설정 일괄 변경 - 설정이 없는 경우 예외")
        void t3() throws Exception {
            // given
            UpdateSettingsRequest request = new UpdateSettingsRequest(
                    Map.of(NotificationSettingType.SYSTEM, false)
            );

            willThrow(new CustomException(ErrorCode.NOTIFICATION_SETTING_NOT_FOUND))
                    .given(settingService).updateSettings(eq(userId), any());
        ...
        @Test
        @DisplayName("알림 설정 일괄 변경 - 인증 토큰 없음")
        void t4() throws Exception {
            ...
            verify(settingService, never()).updateSettings(anyLong(), any());
        }
    }
